...
const AnalysisChart: React.FC<AnalysisChartProps> = ({ data, stepData, isPrediction = false, globalStartDate, globalEndDate, onBrushChange }) => {
  ...
  const formattedStepData = useMemo(() => {
    if (!stepData) return [];
      ...
      <div className="w-full h-[400px] bg-white p-4 rounded-lg shadow-lg mb-8 relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
              ...
              content={({ active, payload, label }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-white p-2 border border-gray-300 rounded shadow">
                      <p className="text-sm font-bold text-black">{`Date: ${label}`}</p>
                      <p className="text-sm text-black">{`Steps: ${payload[0].value}`}</p>
                    </div>
                  );
                }
                return null;
            ...
            />
            <Legend verticalAlign="top" height={36} />
            <Bar dataKey="step" fill={color} name="Steps" />
            {showBrush && (
              <Brush
                dataKey="ds"
                height={30}
  ...
  const stepExplanation = "이 그래프는 시간별 걸음 수를 보여줍니다.";
  ...
  return (
    <div>
      {isPrediction ? (
        renderChart(formattedData, "심박수 BPM", "y", "#FF5733", "sync", predictionExplanation, false)
      ) : (
        <>
      ...
      {stepData && renderStepChart(formattedStepData, "시간별 걸음 수", "#ffa726", "sync", stepExplanation, false)}
    </div>
  );
};
