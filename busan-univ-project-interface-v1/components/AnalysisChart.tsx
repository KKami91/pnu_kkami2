...
import { LineChart, Line, AreaChart, Area, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Brush, TooltipProps } from 'recharts';
...
const AnalysisChart: React.FC<AnalysisChartProps> = ({ 
  ...
  const yAxisDomain = useMemo(() => {
    if (isStep) {
      const values = formattedData.map(item => item[dataKey]).filter(v => v > 0);
      const minValue = Math.min(...values);
      const maxValue = Math.max(...values);
      return [Math.max(1, minValue / 2), maxValue * 1.1]; // Ensure minimum is at least 1 for log scale
    }
    const values = formattedData.map(item => item[dataKey]).filter(value => value != null);
    return [Math.min(...values), Math.max(...values)];
  }, [formattedData, dataKey, isStep]);
        ...
        {isCalorie || isStep ? (
          ...
          >
            ...
            <YAxis
              ...
              domain={yAxisDomain}
              scale={isStep ? 'log' : 'auto'}
            />
            <Tooltip content={<CustomTooltip />} />
            <Legend verticalAlign='top' height={36} />
            <Bar dataKey={dataKey} fill={isCalorie ? '#dd3261' : '#ffa726'} name={isCalorie ? "Calories" : "Steps"} />
            {showBrush && (
              <Brush
                dataKey='ds'
                height={30}
