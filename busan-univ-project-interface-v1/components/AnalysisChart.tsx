...
const AnalysisChart: React.FC<AnalysisChartProps> = ({ 
  ...
  const handleBrushChange = (domain: any) => {
    if (domain && domain.startIndex !== undefined && domain.endIndex !== undefined) {
      const startTime = new Date(data[domain.startIndex].ds).getTime();
      const endTime = new Date(data[domain.endIndex].ds).getTime();
  ...
  const explanation = isStep
    ...
    const ChartComponent = isStep ? BarChart : LineChart;
      ...
      <div className="w-full h-[400px] bg-white p-4 rounded-lg shadow-lg mb-8 relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <YAxis label={{ value: isStep ? 'steps' : (isPrediction ? 'bpm' : 'ms'), angle: -90, position: 'insideLeft' }} />
