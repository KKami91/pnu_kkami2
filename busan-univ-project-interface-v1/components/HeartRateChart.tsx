    ...
    <div className="w-full h-[400px] bg-white p-4 rounded-lg shadow-lg mb-8">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          />
          <YAxis
            tick={{ fill: '#666', fontSize: 12 }}import React from 'react';
            import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Area } from 'recharts';
            ...
            const HeartRateCharts: React.FC<HeartRateChartsProps> = ({ data }) => {
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Line
                        ...
                        strokeWidth={2}
                        connectNulls={true}
                        strokeDasharray={parseISO(item.ds) < predictionStartDate ? "0" : "5 5"}
                      />
                      <Line
                        type="monotone"
                        dataKey="yhat"
              ...
              return (
                <div>
                  {renderChart(fullData, "Complete Heart Rate Data")}
                  {renderChart(fullData.filter(item => parseISO(item.ds) >= predictionStartDate), "Predicted Heart Rate Data (Last 3 Days)")}
                </div>
              );
            };
